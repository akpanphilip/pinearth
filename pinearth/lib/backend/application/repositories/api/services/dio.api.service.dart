...
class DioApiService implements IApiService {
  ...
  DioApiService(this.localStorage) {
    ...
    _dio.interceptors.add(PrettyDioLogger(
      ...
      responseBody: true, responseHeader: true,
      logPrint: (object) {
        if (!kReleaseMode) {
          print(object);
        }
  ...
  @override
  Future<ApiResponse> delete(String endpoint, {bool requireToken = true}) async {
    try {
  ...
  Future<ApiResponse> patch(String endpoint, dynamic data, {
    bool requireToken = true, bool useFormData = true,
  ...
  Future<ApiResponse> post(String endpoint, dynamic data, {
    bool requireToken = true, bool useFormData = false,
    Map<String, dynamic> extraHeader = const {}
  }) async {
    try {
  ...
  ApiResponse _handleResponse(data) {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    ...
    if (options.headers.containsKey('requireToken')) {
      if (options.headers['requireToken'] == true) {
        final token = await localStorage.getItem(userDataBoxKey, userTokenKey, defaultValue: null);
        print(token);
        if (token == null) {
          final error = DioException(requestOptions: options, error: "Unauthenticated");
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    ...
    if (options.headers.containsKey('requireToken')) {
      if (options.headers['requireToken'] == true) {
        ...
        final refreshToken = await localStorage.getItem(userDataBoxKey, userRefreshTokenKey, defaultValue: null);
        if (refreshToken != null) {
          final res = await apiService.post("user/token/refresh/", {
...
class InternetConnectivityInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    ...
    if (![ConnectivityResult.mobile, ConnectivityResult.wifi, ConnectivityResult.other].contains(connectivityResult)) {
      // I am connected to a mobile network.
      final error = DioException(requestOptions: options, error: "Please connect to an internet connection");
      // super.onError(error, handler);
      return handler.reject(error);
    }
    // return options;
...
final dioApiService = Provider<IApiService>((ref) => DioApiService(
  ref.read(hiveLocalStorageService)
));